...
function reset_dialogue_data() {
    ...
    current_dialogue = {"dialogue_id": "", "services": ["test"], "turns": []};
...
function post_dialogue_data() {
    //post_data(post_url, JSON.stringify(dialogue_data));
...
function get_user_state() {
    fetch(backend_url + '/schema', {})
        ...
        current_schema["slots"].forEach(function (item) {
            add_user_state_row(i, item['name'], item['possible_values'][0], item['chinese_description']);
            i += 2;
        });
...
function get_user_state_data() {
    ...
    let user_state = {}
    current_schema["slots"].forEach(function (item) {
        const slot_turn_id = parseInt(document.getElementById('turn-' + item['name']).innerText, 10);
        user_state[slot_turn_id] = {};
        user_state[slot_turn_id][item['name']] = [document.getElementById('input-' + item['name']).value.trim()];
    });
